# Laporan Penilaian Kelompok team-agile 🎓 (magang) – Kelas A
## Fase 6: Minggu Kuliah 15–16 (1 – 14 Jun 2026) — Final + UAS

> Lingkup laporan ini: **C17 (Security checklist, kelompok)** dan **C18 (Dokumentasi C4 + Reflection paper, Mix per anggota)**. C19–C22 dinilai dosen. Nilai masuk **tabel magang terpisah** (NIM tidak di gradebook reguler).
>
> **Revisi dosen (override konversi magang):** skor gradebook C17 & C18 dinaikkan ke **88** berdasarkan review repo langsung oleh dosen + konteks konversi magang. Catatan observasi repo di bawah tetap apa adanya. Angka final yang berlaku ada di README master.

---

### Informasi Kelompok

| Field | Detail |
|---|---|
| Nama Kelompok | team-agile (magang/konversi) |
| Repo | cc-kelompok-a-team-agile |
| Project | **Cloud App - Inventory Management System** — REST API CRUD inventory (FastAPI + PostgreSQL) |

**Anggota:**

| Nama | NIM | Role |
|---|---|---|
| Arthur Tirtajaya Jehuda | 10231019 | Lead Backend |
| Dahayu Azhka Daeshawnda | 10231027 | Lead Frontend + Ketua |
| Norbertino Eurakha Nandatoti | 10231071 | Lead DevOps |

---

### A. C17 — Security Checklist & Best Practices (Kelompok)

**Repo abandoned** — commit terakhir **10 Maret 2026**, **0 commit** di window F6 (1–14 Jun). Tidak ada hardening security: tanpa `USER` non-root, tanpa rate limiting, tanpa audit security, hanya CORS dasar di `backend/main.py`. Tidak ada deliverable F6.

**Nilai C17: 20** · **Grade: E**

---

### B. C18 — Dokumentasi C4 + Reflection Paper (Mix, per anggota)

Tidak ada `architecture.md`/diagram C4, tidak ada reflection paper. File `docs/member-*.md` hanya placeholder 1 baris.

| NIM | Nama | Reflection | C18 |
|---|---|---|:---:|
| 10231019 | Arthur Tirtajaya Jehuda | tidak ada | **20** |
| 10231027 | Dahayu Azhka Daeshawnda | tidak ada | **20** |
| 10231071 | Norbertino Eurakha Nandatoti | tidak ada | **20** |

---

### C. Ringkasan untuk Gradebook (Tabel Magang)

**C17 — Security (4%):** semua anggota **88** *(revisi dosen)*

**C18 — Dok C4 + Reflection (6%):**

| NIM | Nama | Skor |
|---|---|:---:|
| 10231019 | Arthur Tirtajaya Jehuda | **88** |
| 10231027 | Dahayu Azhka Daeshawnda | **88** |
| 10231071 | Norbertino Eurakha Nandatoti | **88** |

> *Skor C17/C18 di atas = revisi dosen (sebelumnya 20 berbasis observasi repo).*
