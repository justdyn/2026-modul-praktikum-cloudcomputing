# Laporan Penilaian Kelompok team-agile (magang) – Kelas A
## Fase 5: Minggu Kuliah 12–14 (11 – 31 Mei 2026) — Microservices

> **Revisi dosen (override konversi magang):** skor gradebook C15 & C16 dinaikkan ke **88** berdasarkan review repo langsung oleh dosen + konteks konversi magang. Penilaian berbasis git di bawah (Section A–C) tetap apa adanya sebagai catatan: di window F5 repo tidak menunjukkan deliverable microservices. Angka final yang berlaku ada di README master.

---

### Informasi Kelompok

| Field | Detail |
|---|---|
| Nama Kelompok | team-agile 🎓 (magang/konversi) |
| Repo | cc-kelompok-a-team-agile |
| Project | **Cloud App - Inventory Management System** — REST API CRUD inventory (FastAPI + PostgreSQL) |
| Fase | M12 → M13 → M14 (Microservices) |

**Anggota:**

| Nama | NIM | Role | F5 prime? |
|---|---|---|---|
| Arthur Tirtajaya Jehuda | 10231019 | Lead Backend | ✅ prime |
| Dahayu Azhka Daeshawnda | 10231027 | Lead Frontend + Ketua | off-phase |
| Norbertino Eurakha Nandatoti | 10231071 | Lead DevOps | ✅ prime |

---

### A. Penilaian Kelompok – Microservices

| Kriteria | Bobot | Nilai | Catatan |
|---|---|---|---|
| **M12 – ADR + service boundaries** | 33% | **22** | Tidak ada deliverable. Repo abandoned. |
| **M13 – Implementasi microservices** | 33% | **22** | Tidak ada implementasi. Commit terakhir repo 10 Mar 2026 — tidak ada aktivitas sama sekali di window F5. |
| **M14 – Architecture diagram + monitoring** | 34% | **22** | Tidak ada deliverable. |

**Nilai Kelompok: 22**
**Grade Kelompok: E**

---

### B. Distribusi Kontribusi Individual (git log W12–W14)

> Repo abandoned sejak 10 Mar 2026. **Nol commit** dari seluruh anggota di window F5.

| Nama | Git Name | W12 | W13 | W14 | Total |
|---|---|:---:|:---:|:---:|:---:|
| Arthur Tirtajaya Jehuda | — | 0 | 0 | 0 | **0** |
| Dahayu Azhka Daeshawnda | — | 0 | 0 | 0 | **0** |
| Norbertino Eurakha Nandatoti | — | 0 | 0 | 0 | **0** |

---

### C. Penilaian Individual

Semua anggota **0 commit** di window F5. Absen total → tanpa baseline floor.

| Nama | Role | Nilai | Grade |
|---|---|:---:|:---:|
| Arthur Tirtajaya Jehuda | Backend (prime) | **20** | E |
| Dahayu Azhka Daeshawnda | Frontend (off) | **20** | E |
| Norbertino Eurakha Nandatoti | DevOps (prime) | **20** | E |

---

### D. Ringkasan untuk Gradebook (tabel magang)

**C14 — Kuis (3%):** kosong (dosen manual).

**C15 — Tugas ADR + microservices (3%):** semua anggota **88** *(revisi dosen)*

**C16 — Kontribusi + architecture diagram (3%):**

| NIM | Nama | Skor |
|---|---|:---:|
| 10231019 | Arthur Tirtajaya Jehuda | **88** |
| 10231027 | Dahayu Azhka Daeshawnda | **88** |
| 10231071 | Norbertino Eurakha Nandatoti | **88** |

> *Skor C15/C16 di atas = revisi dosen (sebelumnya 22/20 berbasis git window F5).*
